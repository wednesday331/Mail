    ...
    fetch(`/emails/${mailbox}`)
        ...
        .then(messages => {
            ...
            messages.forEach(function (email) {
                ...
                groupelement.href='#';
                let messagebody=(mailbox!=='sent' ? `<b>From:</b> ${email.sender} <br/>` : '');
                messagebody+=(mailbox!=='inbox' ? `<b>To:</b> ${email.recipients} <br/>` : '');
                messagebody+=`<b>Subject:</b>` + (email.subject ? email.subject : ` (No Subject)`) + `<br/>`;
                messagebody+=`<span class="badge badge-info badge-pill">${email.timestamp}</span>`;
